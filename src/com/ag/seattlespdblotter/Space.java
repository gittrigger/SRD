...
public class Space extends Activity {
	...
	if(footer){
		{
			...
			/*
			r1.setOnClickListener(new OnClickListener(){public void onClick(View v){Intent lookup = new Intent(mCtx,com.ag.seattlespdblotter.Lookup.class);
	...
	Handler sho = new Handler(){
		...
		public void handleMessage(Message msg){
			...
			rm.setOnClickListener(new OnClickListener(){public void onClick(View v){Intent lookup = new Intent(mCtx,com.ag.seattlespdblotter.Lookup.class);
			lookup.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NO_HISTORY);
			lookup.putExtra("id", moment);
			if(uri != null){lookup.putExtra("uri", uri);}
			startActivity(lookup);wayGo.sendEmptyMessageDelayed(1,750);}});
